...
class User {
    ...
    val profileImagesArrayList = ArrayList<String>()

    companion object {

        fun getUserFromJson(json: JSONObject) : User {

//            기본값 세팅된 User 만들고 => json변수를 이용해 내용물 채우고 => 결과로 return
...
//            사용자의 프사 목록도 파싱해야함.
//            json 변수내의 profile_images JSONArray를 추가로 파싱하자.
...
//                따낸 JSONObject 안에서 이미지 주소 STring만 추출 => user의 프사목록에 추가
                val imageUrl = pfImgObj.getString("img_url")
                user.profileImagesArrayList.add(imageUrl)
            }
